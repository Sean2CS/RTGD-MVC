function [FB,Y,y0] = Construct_FB(X,Y,nClus,nAnch,ks,eta)
%% Setting
nSmp = size(X{1},1);
nView = length(X);
B = cell(nView,1);
...
[y0, ~] = litekmeans(Xs, nClus, 'MaxIter', 100, 'Replicates', 10);
% [~, index] = sort(y0);
% for iView = 1:nView
...
%     Bs = [Bs,B{iView}];
% end
% [y0, ~] = litekmeans(Bs, nClus, 'MaxIter', 100, 'Replicates', 10);
% [~, index] = sort(y0);
% for iView = 1:nView
...
for iView = 1:nView
    for iSmp = 1:nSmp
        [FB{iView}(iSmp,:), ~] = EProjSimplex_new(FB{iView}(iSmp,:), ks);
    end
end
