function [labels,label0,Y0,U,H,conv_lossF,conv_lossM] = HBCD_iter(X, Y0, nClus, nAnch, ks, eta,lambda, delta,TNM)
nView = length(X);
nSmp = size(X{1},1);
%% ============================ Initialization ============================
alpha = cell(nView,1);
[FB,Y0,label0] = Construct_FB(X, Y0, nClus, nAnch, ks, eta);
for iView = 1:nView
...
%% ============================== Upadate H^k =============================
    ...
    Y0 = Y0(index);
    for iView =1:nView
        H{iView} = H{iView}(index,:);
        FB{iView} = FB{iView}(index,:);
        S{iView} = S{iView}(index,:);
        Y{iView} = Y{iView}(index,:);
